...
TEST_F(SdsTest, NoHealthChecker) {
  ...
  // fixfix EXPECT_EQ("hash_5f3725fa79001155", cluster_->versionInfo());
  ...
  EXPECT_CALL(*timer_, enableTimer(_));
  callbacks_->onSuccess(std::move(message));
  //fixfixEXPECT_EQ("hash_2d1af371bb73a287", cluster_->versionInfo());
  EXPECT_EQ(13UL, cluster_->prioritySet().hostSetsPerPriority()[0]->hosts().size());
...
TEST_F(SdsTest, HealthChecker) {
  ...
  EXPECT_CALL(*timer_, enableTimer(_));
  callbacks_->onSuccess(std::move(message));
  //fixfixEXPECT_EQ("hash_5f3725fa79001155", cluster_->versionInfo());
  EXPECT_EQ(13UL, cluster_->prioritySet().hostSetsPerPriority()[0]->hosts().size());
  ...
  callbacks_->onSuccess(std::move(message));
  //fixfixEXPECT_EQ("hash_398882981ada7c10", cluster_->versionInfo());
  ...
  callbacks_->onSuccess(std::move(message));
  //fixfixEXPECT_EQ("hash_b9f8f43f828a3b25", cluster_->versionInfo());
  EXPECT_EQ(13UL, cluster_->prioritySet().hostSetsPerPriority()[0]->hosts().size());
  EXPECT_EQ(12UL, cluster_->prioritySet().hostSetsPerPriority()[0]->healthyHosts().size());
  EXPECT_EQ(12UL, cluster_->info()->stats().membership_healthy_.value());
  EXPECT_EQ(12UL, numHealthy());
...
TEST_F(SdsTest, Failure) {
  ...
  EXPECT_CALL(*timer_, enableTimer(_));
  callbacks_->onSuccess(std::move(message));
  //fixfixEXPECT_EQ("", cluster_->versionInfo());
